...
public class Bully implements IBot {
    ...
    public IMove doMove(IGameState state) {
        int depth = 5; // Set the desired depth
        return calculateWinningMove(state, moveTimeMs, depth);
    }
    ...
    private IMove calculateWinningMove(IGameState state, int maxTimeMs, int depth){
        ...
        while (System.currentTimeMillis() < time + maxTimeMs) { // check how much time has passed, stop if over maxTimeMs
            ...
            if (simulator.getGameOver() == GameOverState.Win) {
                int score = scoreWin(simulator.getCurrentState(), simulator.getCurrentState().getMoveNumber(), simulator, depth);
                if (score > bestScore) {
                    bestScore = score;
    ...
    private IMove selectMoveBasedOnHeuristics(IGameState state, List<IMove> moves) {
        ...
        for (IMove move : moves) {
            int score = 0;
            if (formsThreeInALine(state, move)) score += 5;
            if (blocksOpponentThreeInALine(state, move)) score += 4;
            if (isCenterPosition(move)) score += 3;
            if (createsForkOpportunity(state, move)) score += 2;
    ...
    private int scoreWin(IGameState finalState, int moveCount, GameSimulator simulator, int depth) {
        int score = 0;

        // Win speed based on move count and depth
        score += (100 - moveCount - depth * 10);

        // Add evaluation based on macroboard
        String[][] macroboard = finalState.getField().getMacroboard();
        for (int i = 0; i < macroboard.length; i++) {
            for (int j = 0; j < macroboard[i].length; j++) {
                if (macroboard[i][j].equals("0")) {
                    score += 10; // Add score if a macroboard position is won by the bot
                } else if (macroboard[i][j].equals("1")) {
                    score -= 10; // Subtract score if a macroboard position is won by the opponent
                }
                // You can add more nuanced scoring based on the state of the macroboard if needed
            }
        }
